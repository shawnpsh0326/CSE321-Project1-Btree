...
private:
    ...
    // split 전에 redistribution 시도하는 함수
    bool redistribution(Node* parent, int index) {
        Node* left = nullptr;
        Node* right = nullptr;

        // 오른쪽 node가 가능하면 오른쪽으로 시도
        if (index + 1 < parent->children.size() && parent->children[index + 1]->keys.size() < maxKeys()) {
            left = parent->children[index];
            right = parent->children[index + 1];
        }
        // 오른쪽이 안되면 왼쪽 node 시도
        else if (index > 0 && parent->children[index - 1]->keys.size() < maxKeys()) {
            index--;
            left = parent->children[index];
            right = parent->children[index + 1];
        }
        ...
        // 일단 node의 key, rid, children(internal node일 때)를 전부 합침.
        vector<int> totalKeys;
        vector<int> totalRids;
        vector<Node*> totalChildren;

        for (int i = 0; i < left->keys.size(); i++) {
            totalKeys.push_back(left->keys[i]);
            totalRids.push_back(left->rids[i]);
        }

        totalKeys.push_back(parent->keys[index]);
        totalRids.push_back(parent->rids[index]);

        for (int i = 0; i < right->keys.size(); i++) {
            totalKeys.push_back(right->keys[i]);
            totalRids.push_back(right->rids[i]);
        }

        if (!left->isLeaf) {
            for (int i = 0; i < left->children.size(); i++) {
                totalChildren.push_back(left->children[i]);
            }

            for (int i = 0; i < right->children.size(); i++) {
                totalChildren.push_back(right->children[i]);
            }
        }

        // 현재 node에 들어갈 개수 
        int leftNum = totalKeys.size() / 2;
        ...
        for (int i = 0; i < leftNum; i++) {
            left->keys.push_back(totalKeys[i]);
            left->rids.push_back(totalRids[i]);
        }
        if (!left->isLeaf) {
            for (int i = 0; i <= leftNum; i++) {
                left->children.push_back(totalChildren[i]);
            }
        }

        // parent 채우기
        parent->keys[index] = totalKeys[leftNum];
        parent->rids[index] = totalRids[leftNum];

        // 오른쪽 node 채우기
        for (int i = leftNum + 1; i < totalKeys.size(); i++) {
            right->keys.push_back(totalKeys[i]);
            right->rids.push_back(totalRids[i]);
        }
        if (!left->isLeaf) {
            for (int i = leftNum + 1; i < totalChildren.size(); i++) {
                right->children.push_back(totalChildren[i]);
            }
        }
    ...
    // redistribution이 불가능할 때 sibling과 2-to-3 split하는 함수
    bool TTT(Node* parent, int index) {
        Node* left = nullptr;
        Node* right = nullptr;
        ...
        // 각 node에 들어갈 개수 구하기
        int totalKeysNum = totalKeys.size() - 2;
        int leftNum = (totalKeysNum + 2) / 3;
        int middleNum = (totalKeysNum + 1) / 3;

        // 2 to 3 이므로 새로운 node 하나 생성
        ...
        for (int i = leftNum + 1; i < leftNum + 1 + middleNum; i++) {
            middle->keys.push_back(totalKeys[i]);
            middle->rids.push_back(totalRids[i]);
        }

        parent->keys.insert(parent->keys.begin() + index + 1, totalKeys[leftNum + 1 + middleNum]);
        parent->rids.insert(parent->rids.begin() + index + 1, totalRids[leftNum + 1 + middleNum]);

        for (int i = leftNum + 1 + middleNum + 1; i < totalKeys.size(); i++) {
            right->keys.push_back(totalKeys[i]);
            right->rids.push_back(totalRids[i]);
        }
        ...
        if (!left->isLeaf) {
            ...
            for (int i = leftNum + 1; i <= leftNum + 1 + middleNum; i++) {
                middle->children.push_back(totalChildren[i]);
            }

            for (int i = leftNum + 1 + middleNum + 1; i < totalChildren.size(); i++) {
                right->children.push_back(totalChildren[i]);
            }
        }

        parent->children.insert(parent->children.begin() + index + 1, middle);
    ...
    void insert(Node* node, int key, int rid) {
        ...
        while (node->keys.size() > maxKeys()) {
            ...
            if (!path.empty()) {
                ...
                // 먼저 redistribution 시도
                if (redistribution(parent, index))
                    break;

                // redistribution 실패하면 2-to-3 split 시도
                if (TTT(parent, index)) {
                    path.pop_back();
                    pathIndex.pop_back();
                    node = parent;
            ...
            // root split하는 경우는 b_tree처럼 기본 split
            splitNum++;
...
int main() {
    ...
    while (!exit) {
        ...
        BStarTree b_star_tree(d);

        // insert 시간 측정
        auto insertStart = chrono::high_resolution_clock::now();
        for (int i = 0; i < students.size(); i++) {
            b_star_tree.insert(students[i].id, i);
        }
        ...
        for (int i = 0; i < queryNum; i++) {
            ...
            int rid = b_star_tree.search(key);
        ...
        vector<int> rangeRids = b_star_tree.rangeQuery(startKey, endKey);
        ...
        for (int i = 0; i < deleteNum; i++) {
            ...
            b_star_tree.remove(key);

            if (b_star_tree.search(key) == -1)
                successDeleteNum++;
        }
        ...
        cout << "Node utilization: " << b_star_tree.utilization() * 100 << "%" << endl;
        cout << "Number of splits: " << b_star_tree.getSplitNum() << endl << endl;
